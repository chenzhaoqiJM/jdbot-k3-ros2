...
inline std::string createRuntimeCameraSettings(
    ...
    const sensor_msgs::msg::CameraInfo &camera_info, bool resize_for_orb,
    int tracking_width, int tracking_height, const std::string &node_name) {
  ...
  const int output_width =
      resize_for_orb ? tracking_width : static_cast<int>(camera_info.width);
  const int output_height =
      resize_for_orb ? tracking_height : static_cast<int>(camera_info.height);
  const double scale_x = static_cast<double>(output_width) / camera_info.width;
  const double scale_y = static_cast<double>(output_height) / camera_info.height;
  const auto distortion = [&camera_info](std::size_t index) {
    return index < camera_info.d.size() ? camera_info.d[index] : 0.0;
  };

  const std::unordered_map<std::string, std::string> replacements{
      {"Camera1.fx", formatNumber(camera_info.k[0] * scale_x)},
      {"Camera1.fy", formatNumber(camera_info.k[4] * scale_y)},
      {"Camera1.cx", formatNumber(camera_info.k[2] * scale_x)},
      {"Camera1.cy", formatNumber(camera_info.k[5] * scale_y)},
      ...
      {"Camera.width", std::to_string(output_width)},
      {"Camera.height", std::to_string(output_height)},
  };
