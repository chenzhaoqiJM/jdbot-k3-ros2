...
class OrbSlam3SlamNode final : public rclcpp::Node {
 ...
 private:
  void cameraInfoCallback(
    ...
    try {
      runtime_settings_ = orbslam3_ros2::createRuntimeCameraSettings(
          settings_template_, *camera_info, resize_for_orb_, tracking_width_,
          tracking_height_, get_name());
  ...
  void imageCallback(const Image::ConstSharedPtr &color_msg,
    ...
    try {
      ...
      if (resize_for_orb_ &&
        ...
        cv::resize(color, resized_color, tracking_size, 0.0, 0.0,
                   cv::INTER_AREA);
